CREATE DEFINER=`root`@`localhost` PROCEDURE `Payment`(IN 处方ID int,
                                                      IN 操作员ID int,
																											OUT 结果 varchar(10))
BEGIN
  declare cur int default 1;
	...
	update patientmediprescription P set PrescriptionStatus = 2 where P.id = 处方ID;
	update patientmedipredetail PD set `Status` = 2 where PD.MediPrescriptionID = 处方ID;
